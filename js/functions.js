    ...
    $(".music-list").on("click", ".icon-play,.icon-download,.icon-share,.icon-share-click", function () {
        ...
        switch ($(this).data("function")) {
            ...
            case "share":   // 分享
                ...
                share(musicList[rem.dislist].item[num]);
                break;
            case "cookie":   // 收藏
                var url = musicList[rem.dislist].item[num].url;
                if (url.includes('/kw/')) {
...
function musicInfo(list, index) {
    ...
    if (music.url.includes('/kw/')) {
        ...
        checkCookieBySourceId('kwmusic', kuwoid);
    } else {
...
function searchBox() {
    var tmpHtml = '<form onSubmit="return searchSubmit()"><div id="search-area">' +
        ...
        '        <input type="text" name="wd" id="search-wd" placeholder="搜尋歌手、歌名、專輯" autofocus required>' +
...
function searchSubmit() {
    ...
    if (!wd) {
        layer.msg('搜尋內容不能為空', { anim: 6, offset: 't' });
        $("#search-wd").focus();
        return false;
    }
...
function thisDownload(obj) {
    ...
    music = musicList[$(obj).data("list")].item[$(obj).data("index")];
    download(music)
}
...
function thisMusic(obj) {
    var url = musicList[$(obj).data("list")].item[$(obj).data("index")].url
    if (url.includes('/kw/')) {
        var kuwoid = url.split('/kw/')[1].replace('.mp3', '');
        setCookieBySourceId('kwmusic', kuwoid);
    } else {
...
function download(music) {
    if (music.url.includes('/kw/')) {
        ...
        music.url = music.url;
        openDownloadDialog(music.url, music.name + '_' + music.artist + '.mp3');
    }
...
function openDownloadDialog(url, saveName) {
    ...
    //     event.MouseEvent('click', { bubbles: true, cancalable: false, view: window, detail: 0, screenX: 0, screenY: 0, clientX: 0, clientY: 0, ctrlKey: false, altKey: false, shiftKey: false, metaKey: false, button: 0, relatedTarget: null });
    // }

    var event = new MouseEvent('click', { bubbles: true, cancalable: false, view: window, detail: 0, screenX: 0, screenY: 0, clientX: 0, clientY: 0, ctrlKey: false, altKey: false, shiftKey: false, metaKey: false, button: 0, relatedTarget: null });
...
function switchPl(id) {
    ...
    switch (id) {
        ...
        case 'myTimeMusicConcert': clearSheet(); musicList = DefaultMusicList; musicList = musicList.concat(myTimeMusicConcert); initList(); dataBox("sheet"); break;
        case 'myDreamVoice': clearSheet(); musicList = DefaultMusicList; musicList = musicList.concat(myDreamVoice); initList(); dataBox("sheet"); break;
